...
class FWHMAnalyzerApp:
    def __init__(self, master):
        self.master = master
        master.title("FWHM 解析プログラム - Step 2改: 対話的フィッティング")
    ...
    def save_results(self):
        ...
        initial_file = f"{base_name}_after.xlsx" if base_name.endswith("_after") else f"{base_name}_after_after.xlsx"
...
class AnalysisWindow:
    def __init__(self, parent, data, title):
        ...
        self.y_smooth = savgol_filter(self.y_raw, window_size, 2) # 次数を2に下げて安定化
        ...
        self.range_span = self.ax.axvspan(self.x.min(), self.x.max(), color='yellow', alpha=0.3)
        self.fwhm_text = self.ax.text(0.05, 0.95, '', transform=self.ax.transAxes, fontsize=12, verticalalignment='top', bbox=dict(boxstyle='round,pad=0.5', fc='wheat', alpha=0.5))
        self.ax.legend()
        self.ax.grid(True)
        self.ax.set_xlabel("Wavelength (nm)")
        self.ax.set_ylabel("Intensity")
        self.fig.tight_layout(rect=[0, 0.05, 1, 1]) # スライダー領域を確保

        # --- Range Slider と軸の位置同期 ---
        self.slider_ax = self.fig.add_axes([0.1, 0.02, 0.8, 0.03])
        self.slider = RangeSlider(self.slider_ax, "Fit Range", self.x.min(), self.x.max())
        self.slider.on_changed(self.update_fit)
        
        self.canvas.draw()
        ax_pos = self.ax.get_position()
        self.slider_ax.set_position([ax_pos.x0, 0.02, ax_pos.width, 0.03])
    ...
    def pseudo_voigt(self, x, A, mu, sigma_g, alpha):
        sigma_l = alpha * sigma_g
        return A * ((1-alpha) * (1 / (sigma_g * np.sqrt(2 * np.pi))) * np.exp(-((x - mu) ** 2) / (2 * sigma_g ** 2)) + \
               alpha * (1 / np.pi) * (sigma_l / ((x - mu) ** 2 + sigma_l ** 2)))
        
    def update_fit(self, val):
        min_val, max_val = val
        self.range_span.set_xy([[min_val, 0], [min_val, 1], [max_val, 1], [max_val, 0], [min_val, 0]])
        ...
        try:
            ...
            mu0 = x_fit[peak_idx]
            A0 = y_fit[peak_idx] * 20 # 面積の初期値として
            sigma_g0 = (x_fit[-1] - x_fit[0]) / 5
            alpha0 = 0.5
            
            p0 = [A0, mu0, sigma_g0, alpha0]
            ...
            # FWHMの計算
            peak_intensity = self.pseudo_voigt(popt[1], *popt)
            half_max = peak_intensity / 2.0
            
            # 半値幅となる左右のx座標を探索
            left_half_x = self.find_intersect(self.x, y_fitted_full, half_max, 'left', popt[1])
            right_half_x = self.find_intersect(self.x, y_fitted_full, half_max, 'right', popt[1])
            
            if left_half_x is not None and right_half_x is not None:
                self.fwhm_result = right_half_x - left_half_x
            else:
                self.fwhm_result = float('nan')

            self.line_fit.set_ydata(y_fitted_full)
            self.fwhm_text.set_text(f'FWHM: {self.fwhm_result:.3f} nm' if not np.isnan(self.fwhm_result) else 'FWHM: N/A')
    ...
    def find_intersect(self, x_data, y_data, level, side, center_x):
        if side == 'left':
            indices = np.where((x_data <= center_x) & (np.diff(np.sign(y_data - level)) != 0))[0]
        ...
        x1, y1 = x_data[idx], y_data[idx]
        x2, y2 = x_data[idx+1], y_data[idx+1]
        return np.interp(level, [y1, y2], [x1, x2])
