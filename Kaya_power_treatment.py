...
class AnalysisModel:
    ...
    def analyze_by_gradient(self, high_factor: float, low_factor: float):
        ...
        if low_threshold >= high_threshold:
            raise ValueError("低い閾値は高い閾値より小さく設定してください。")

        labels = np.full(y_log.shape, 'Stair')
        in_transition = False
        for i, grad_val in enumerate(processed_gradient):
            if not in_transition and grad_val > high_threshold:
                in_transition = True
            elif in_transition and grad_val < low_threshold:
                in_transition = False
...
class AnalysisView:
    def __init__(self, root: tk.Tk, controller):
        ...
        self.gradient_high_factor_var = tk.StringVar(value="3.0")
        self.gradient_low_factor_var = tk.StringVar(value="1.0")
    ...
    def _create_parameter_inputs(self, parent: ttk.Frame):
        ...
        self.grad_frame.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(10,0))

        ttk.Label(self.grad_frame, text="High Threshold Factor (↓ sensitive):").grid(row=0, column=0, sticky="w", pady=2)
        ttk.Entry(self.grad_frame, textvariable=self.gradient_high_factor_var, width=10).grid(row=0, column=1, sticky="w", padx=5)

        ttk.Label(self.grad_frame, text="Low Threshold Factor (↑ sensitive):").grid(row=1, column=0, sticky="w", pady=2)
        ttk.Entry(self.grad_frame, textvariable=self.gradient_low_factor_var, width=10).grid(row=1, column=1, sticky="w", padx=5)
    ...
    def get_ui_parameters(self) -> dict:
        try:
            params = {
                ...
                "gradient_high_factor": float(self.gradient_high_factor_var.get()),
                "gradient_low_factor": float(self.gradient_low_factor_var.get()),
    ...
    def _create_graph_panel(self, parent: ttk.Frame):
        ...
        toolbar.update()
        self.ax.set_title('Step Data Analysis')
        self.canvas.draw()
    ...
    def redraw_plot(self, model: AnalysisModel):
        ...
        self.ax.set_title('Step Data Analysis Results')
        self.ax.set_xlabel('Data Point Index')
        self.ax.set_ylabel('Intensity (μJ)')
    ...
    def update_segment_buttons(self, model: AnalysisModel):
        ...
        style = ttk.Style()
        style.configure("SelectedSegment.TButton", borderwidth=2, relief="sunken")
        ...
        for s in model.segments:
            if s['type'] == 'Stair':
                ...
                if s['id'] == model.selected_segment_id:
                    btn.config(style="SelectedSegment.TButton")
...
class AnalysisController:
    def __init__(self, root: tk.Tk):
        ...
        s = ttk.Style()
        s.configure('Accent.TButton', font=('calibri', 10, 'bold'), foreground='white', background='#0078D7')
    ...
    def run_analysis(self):
        ...
        try:
            if not params['filepath']:
                raise ValueError("データファイルが選択されていません。")
            ...
            self.model.analyze_by_gradient(
                params['gradient_high_factor'],
                params['gradient_low_factor']
            )
